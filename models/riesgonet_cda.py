...
class FinancieraRiesgonetCda(models.Model):
	_name = 'financiera.riesgonet.cda'

	_order = 'orden asc'
	name = fields.Char('Nombre')
	activo = fields.Boolean('Activo')
	orden = fields.Integer('Orden de ejecucion')
	otorgar_cpm_base = fields.Float('Riesgonet - CPM Base', digits=(16,2))
	otorgar_cpm_maximo = fields.Float('Riesgonet - CPM Maximo', digits=(16,2))
	otorgar_partner_tipo_id = fields.Many2one('financiera.partner.tipo', 'Tipo de cliente')
	regla_ids = fields.One2many('financiera.riesgonet.cda.regla', 'riesgonet_cda_id', 'Reglas')
	...
	def ejecutar(self, informe_id):
		...
		cda_resultado_id = self.env['financiera.riesgonet.cda.resultado'].create({
			'name': self.name,
		...
		for regla_id in self.regla_ids:
			...
			regla_resultado_id.riesgonet_cda_id = None
			regla_resultado_id.riesgonet_cda_resultado_id = cda_resultado_id.id
...
class FinancieraRiesgonetCdaResultado(models.Model):
	_name = 'financiera.riesgonet.cda.resultado'

	_order = 'id desc'
	name = fields.Char('Nombre')
	informe_id = fields.Many2one('financiera.riesgonet.informe', 'Informe')
	regla_ids = fields.One2many('financiera.riesgonet.cda.regla', 'riesgonet_cda_resultado_id', 'Reglas')
	otorgar_cpm = fields.Float('Riesgonet - CPM', digits=(16,2))
	otorgar_cpm_maximo = fields.Float('Riesgonet - CPM Maximo', digits=(16,2))
	otorgar_partner_tipo_id = fields.Many2one('financiera.partner.tipo', 'Tipo de cliente')
	resultado = fields.Selection([('rechazado', 'Rechazado'), ('aprobado', 'Aprobado')], 'Resultado')

class FinancieraRiesgonetCdaRegla(models.Model):
	_name = 'financiera.riesgonet.cda.regla'

	riesgonet_cda_id = fields.Many2one('financiera.riesgonet.cda', 'CDA Evaluacion')
	riesgonet_cda_resultado_id = fields.Many2one('financiera.riesgonet.cda.resultado', 'CDA Resultado')
	...
	def ejecutar(self, informe_id):
		self.ensure_one()
		variable_obj = self.pool.get('financiera.riesgonet.informe.variable')
		...
		if len(variable_ids) > 0:
			...
			if self.resultado == 'aprobado':
				self.riesgonet_cda_resultado_id.otorgar_cpm *= self.cpm_multiplicar
